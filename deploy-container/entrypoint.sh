...
if [[ -z "${RCLONE_DATA}" ]]; then
    update_status "RCLONE_DATA is not specified. Files will not persist"

    # start the project
    project_init
else
    update_status "Configuring rclone..."
    ...
    if rclone ls $RCLONE_REMOTE_PATH; then
        if [ $RCLONE_AUTO_PULL = "true" ]; then
            ...
            update_status "Pulling existing files from remote..."
            /home/coder/pull_remote.sh
            update_status "Files pulled from remote"
        else
            update_status "Auto-pull is disabled"
        fi
    else
        if [ $RCLONE_AUTO_PUSH = "true" ]; then
            # we need to clone the git repo and sync
            update_status "Pushing initial files to remote..."
            project_init
            /home/coder/push_remote.sh
            update_status "Initial files pushed to remote"
        else
            update_status "Auto-push is disabled"
            project_init
        fi
    fi
fi
